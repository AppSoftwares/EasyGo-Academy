...
export default function ConversationPractice({ onEarnXp, userLevel, isDarkMode }: ConversationPracticeProps) {
  ...
  const inputClass = isDarkMode ? 'bg-zinc-950 border-white/10 text-white placeholder:text-slate-400' : 'bg-white/5 border border-slate-200 text-slate-900 placeholder:text-slate-500';

  return (
    <div id="practice-container" className={`space-y-6 ${pageBgClass}`}>
      {/* Title */}
      <div className="flex flex-col gap-1.5">
        <span className="font-academy text-brand-orange text-3xl font-semibold">Pierde el Miedo</span>
        <h1 className={`font-display font-extrabold text-2xl tracking-tight ${pageTextClass}`}>Práctica Oral Interactiva</h1>
        <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Elige entre entablar una conversación realista o jugar con un Host de IA carismático.</p>
      </div>
      ...
      {activePracticeTab === 'scenarios' ? (
          ...
          <div id="scenarios-practice-box" className="glass rounded-3xl border border-white/10 overflow-hidden flex flex-col min-h-[500px]">
            ...
            <div className="bg-white/5 border-b border-white/5 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div>
                ...
                <h3 className={`text-sm font-bold shrink-0 ${pageTextClass}`}>{selectedScenario}</h3>
              </div>
            ...
            <div className="bg-white/5 border-t border-white/5 p-4 flex items-center gap-3">
              ...
              <input
                ...
                className={`flex-1 rounded-full px-5 py-3 text-sm focus:outline-none focus:border-brand-orange transition-all ${inputClass}`}
              />
        ...
        <>
          <div id="trivia-practice-box" className="space-y-4">
            {!triviaStarted ? (
              <div className="glass rounded-3xl p-6 border border-white/10 space-y-6"> {/* Personality choices grid */}
                <div className="text-center space-y-1">
                  <Bot className="w-10 h-10 text-brand-orange mx-auto mb-2 animate-float" />
                  <h3 className={`text-lg font-bold font-display ${pageTextClass}`}>Elige la personalidad de tu Host de IA</h3>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  {PLAYABLE_PERSONALITIES.map((pers) => (
                    ...
                    >
                      ...
                      <h4 className={`text-xs font-bold ${pageTextClass}`}>{pers.name}</h4>
              ...
              <div className="glass rounded-3xl border border-white/10 p-6 space-y-6 relative overflow-hidden"> {/* TRIVIA QUESTIONS VIEWPLAY */}
                ...
                <div className="flex justify-between items-center bg-white/5 p-3 rounded-2xl border border-white/5">
                  <div className="flex gap-1.5 items-center">
                    ...
                    <div className="text-left font-display">
                      ...
                      <span className={`text-xs font-bold block ${pageTextClass}`}>{selectedPersonality.name.split(",")[0]}</span>
                    </div>
                  </div>
                ...
                <div className="flex items-start gap-3 bg-white/5 rounded-2xl p-4 border border-white/5 relative">
                  ...
                  <div className="space-y-1.5">
                    <p className={`text-xs leading-relaxed font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                      {triviaHostReply}
                    </p>
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                    ...
                    <h3 className={`text-md font-bold mt-1 leading-snug ${pageTextClass}`}>{triviaQuestion}</h3>
                  </div>
